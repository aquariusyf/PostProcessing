...
def plotMediaKPI(Plot_Title = '', Plot_Attributes = {}, Plot_Peer_Side_UL_Attributes = {}, plot = -1):
    ...
    if Plot_Attributes != {}:
        ...
        plot.scatter(pd.to_datetime(Plot_Attributes[X_CRC_PASS], format="%H:%M:%S.%f"), Plot_Attributes[Y_CRC_PASS], label = "PDSCH CRC PASS", color='limegreen', marker='.')
        plot.scatter(pd.to_datetime(Plot_Attributes[X_CRC_FAIL], format="%H:%M:%S.%f"), Plot_Attributes[Y_CRC_FAIL], label = "PDSCH CRC FAIL", color='red', marker='.')
        plot.step(pd.to_datetime(Plot_Attributes[X_TALKSPURT], format="%H:%M:%S.%f"), Plot_Attributes[Y_TALKSPURT], label = "Talkspurt", color='dodgerblue', marker='.')
        plot.scatter(pd.to_datetime(Plot_Attributes[X_HO_START], format="%H:%M:%S.%f"), Plot_Attributes[Y_HO_START], label = "HO Start", color='black', marker='>')
        plot.scatter(pd.to_datetime(Plot_Attributes[X_HO_SUC],format="%H:%M:%S.%f"), Plot_Attributes[Y_HO_SUC], label = "HO Success", color='black', marker='s')
        plot.scatter(pd.to_datetime(Plot_Attributes[X_PKT_BURST],format="%H:%M:%S.%f"), Plot_Attributes[Y_PKT_BURST], label = "DL RTP Burst", color='darkorange', marker='P')
        plot.scatter(pd.to_datetime(Plot_Attributes[X_PKT_LOSS],format="%H:%M:%S.%f"), Plot_Attributes[Y_PKT_LOSS], label = "DL Pkt Loss", color='red', marker='x')
    if Plot_Peer_Side_UL_Attributes != {}:
        plot.step(pd.to_datetime(Plot_Peer_Side_UL_Attributes[X_UL_RTP], format="%H:%M:%S.%f"), Plot_Peer_Side_UL_Attributes[Y_UL_RTP], label = "FarEnd UL RTP", color='blueviolet', marker='.' )
        plot.scatter(pd.to_datetime(Plot_Peer_Side_UL_Attributes[X_UL_NEW_TX], format="%H:%M:%S.%f"), Plot_Peer_Side_UL_Attributes[Y_UL_NEW_TX], label = "FarEnd PUSCH NEW-TX", color='aqua', marker='.')
        plot.scatter(pd.to_datetime(Plot_Peer_Side_UL_Attributes[X_UL_RE_TX],format="%H:%M:%S.%f"), Plot_Peer_Side_UL_Attributes[Y_UL_RE_TX], label = "FarEnd PUSCH RE-TX", color='red', marker='.')

from PostProcessingUtils import LogPacket_Talkspurt, LogPacket_RTP, LogPacket_PHY_BLER, PostProcessingUtils
...
for logName in LogPkt_All.keys():
    ...
    Plot_Attr_Single[X_PKT_BURST] = PktBurst.keys()
    ...
    for rtpPkt in RTP_Packet_List:
        if rtpPkt.getPacketCode() == '0x1568':
            if rtpPkt.getDirection() == 'NETWORK_TO_UE' and rtpPkt.getMediaType() == 'AUDIO' and rtpPkt.isInTalkspurt():
                ...
                Plot_Attr_Single[X_TALKSPURT].append(rtpPkt.getAbsTime())
                ...
                if rtpPkt.getPayloadSize() >= 30:
                    ...
                    Plot_Attr_Single[X_UL_RTP].append(rtpPkt.getAbsTime())
                else:
            ...
            Plot_Attr_Single[X_PKT_LOSS].append(rtpPkt.getAbsTime())           
    ...
    for logPkt in LogPkt_All[logName]:
        if logPkt.getTitle() == 'Event  --  EVENT_NR5G_RRC_HO_STARTED_V2':
            ...
            Plot_Attr_Single[X_HO_START].append(logPkt.getAbsTime())
        elif logPkt.getTitle() == 'Event  --  EVENT_NR5G_RRC_HO_SUCCESS':
            Plot_Attr_Single[Y_HO_SUC].append(HO_SUC)
            Plot_Attr_Single[X_HO_SUC].append(logPkt.getAbsTime())
...
def plotMediaKPI(Plot_Title = '', Plot_Attributes = {}, Plot_Peer_Side_UL_Attributes = {}, plot = -1):
    ...
    #plot.step(Plot_Attributes[X_CDRX], Plot_Attributes[Y_CDRX], label = "CDRX", color='magenta')
    plot.scatter(Plot_Attributes[X_CRC_PASS], Plot_Attributes[Y_CRC_PASS], label = "PDSCH CRC PASS", color='limegreen', marker='.')
    plot.scatter(Plot_Attributes[X_CRC_FAIL], Plot_Attributes[Y_CRC_FAIL], label = "PDSCH CRC FAIL", color='red', marker='.')  
    plot.step(Plot_Attributes[X_TALKSPURT], Plot_Attributes[Y_TALKSPURT], label = "Talkspurt", color='dodgerblue', marker='.')
    plot.step(Plot_Peer_Side_UL_Attributes[X_UL_RTP], Plot_Peer_Side_UL_Attributes[Y_UL_RTP], label = "FarEnd UL RTP", color='blueviolet', marker='.' )
    plot.scatter(Plot_Peer_Side_UL_Attributes[X_UL_NEW_TX], Plot_Peer_Side_UL_Attributes[Y_UL_NEW_TX], label = "FarEnd PUSCH NEW-TX", color='aqua', marker='.')
    plot.scatter(Plot_Peer_Side_UL_Attributes[X_UL_RE_TX], Plot_Peer_Side_UL_Attributes[Y_UL_RE_TX], label = "FarEnd PUSCH RE-TX", color='red', marker='.')
    plot.scatter(Plot_Attributes[X_HO_START], Plot_Attributes[Y_HO_START], label = "HO Start", color='black', marker='>')
    plot.scatter(Plot_Attributes[X_HO_SUC], Plot_Attributes[Y_HO_SUC], label = "HO Success", color='black', marker='s')
    plot.scatter(Plot_Attributes[X_PKT_BURST], Plot_Attributes[Y_PKT_BURST], label = "DL RTP Burst", color='darkorange', marker='P')
    plot.scatter(Plot_Attributes[X_PKT_LOSS], Plot_Attributes[Y_PKT_LOSS], label = "DL Pkt Loss", color='red', marker='x')  
